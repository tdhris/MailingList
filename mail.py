...
def process_input(command):
	if command == 'exit':
		...
		arguments = assist_functions.get_arguments(command, 2)
		list_index, subscriber_index = arguments[0], arguments[1]
		updated_person = add_person()
		assist_functions.update_subscriber_info(list_index, subscriber_index, updated_person)

	elif command.startswith("remove_subscriber"):
		arguments = assist_functions.get_arguments(command, 2)
		list_index, subscriber_index = arguments[0], arguments[1]
		assist_functions.remove_subscriber(list_index, subscriber_index)

	elif command.startswith("update "):
		arguments = assist_functions.get_arguments(command, 2)
		list_index, new_list_name = arguments[0], arguments[1]
		assist_functions.change_list_name(list_index, new_list_name)
